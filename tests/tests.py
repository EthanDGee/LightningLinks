import unittest
from src.main import find_min_from_indexes, get_top_n_similarities_from_row


class TestFindMinFromIndexes(unittest.TestCase):
    def test_single_element(self):
        # Single element in indexes
        self.assertEqual(find_min_from_indexes([2], [4, 5, 1, 7]), 2)
        self.assertEqual(find_min_from_indexes([0], [8, 6, 2, 1]), 0)
        self.assertEqual(find_min_from_indexes([3], [1, 3, 5, 0]), 3)

    def test_multiple_elements(self):
        # Multiple elements in indexes
        self.assertEqual(find_min_from_indexes([1, 3, 4], [8, 3, 6, 2, 5]), 3)
        self.assertEqual(find_min_from_indexes([0, 2, 3], [4, 7, 2, 1]), 3)
        self.assertEqual(find_min_from_indexes([2, 5, 6], [9, 12, 7, 15, 10, 3, 2]), 6)

    def test_min_at_start(self):
        # Minimum at the start of indexes
        self.assertEqual(find_min_from_indexes([0, 2, 4], [1, 7, 3, 9, 10]), 0)
        self.assertEqual(find_min_from_indexes([1, 2, 3], [3, 5, 9, 12]), 1)
        self.assertEqual(find_min_from_indexes([0, 3, 4], [2, 7, 9, 3, 8]), 0)

    def test_min_at_end(self):
        # Minimum at the end of indexes
        self.assertEqual(find_min_from_indexes([1, 3, 5, 6], [9, 12, 8, 15, 10, 6, 3]), 6)
        self.assertEqual(find_min_from_indexes([0, 4, 5, 8], [8, 12, 15, 18, 3, 5, 6, 7, 1]), 8)
        self.assertEqual(find_min_from_indexes([2, 3, 7], [9, 8, 5, 10, 11, 15, 18, 3]), 7)
